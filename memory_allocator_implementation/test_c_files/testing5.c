...
int main() {
   ...
   init_allocator(virtual_heap, 8, 4);
   virtual_info(virtual_heap);
   printf("\n");
   virtual_malloc(virtual_heap, 15);
   virtual_info(virtual_heap);
   printf("\n");
   virtual_malloc(virtual_heap, 64);
   virtual_info(virtual_heap);
   if(virtual_malloc(virtual_heap, 129) == NULL) {
      printf("Error: couldn't allocate 129 bytes\n");
   }
   if (virtual_malloc(virtual_heap, 256) == NULL) {
      printf("Error: couldn't allocate 256 bytes\n");
   }
