...
int main() {
   ...
   init_allocator(virtual_heap, 25, 21);
   virtual_info(virtual_heap);
   printf("\n");
   virtual_malloc(virtual_heap, 16777216);
   virtual_info(virtual_heap);
   printf("\n");
   virtual_malloc(virtual_heap, 4194304);
   virtual_info(virtual_heap);
   printf("\n");
   virtual_malloc(virtual_heap, 8388608);
   virtual_info(virtual_heap);
   printf("\n");
   if (virtual_realloc(virtual_heap, (((((struct handler*)virtual_heap)->data_structures)->next)->next)->address, 8388608 + 1) == NULL) {
      printf("Error: unable to allocate in virtual_realloc. Returned (NULL)\n\n");
   }
   virtual_info(virtual_heap);
